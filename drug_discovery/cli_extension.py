...
def register_commands(subparsers):
    ...
    p.add_argument("--use-amp",action="store_true",default=True); p.add_argument("--use-ema",action="store_true",default=True)
    p.add_argument("--scheduler",choices=["cosine_warm_restarts","plateau","one_cycle"],default="cosine_warm_restarts")
    p.set_defaults(func=cmd_train_advanced)

    p=subparsers.add_parser("evaluate-scientific",help="Scaffold k-fold CV with stats")
    p.add_argument("--model",default="egnn"); p.add_argument("--dataset",required=True)
    p.add_argument("--n-folds",type=int,default=5); p.add_argument("--seed",type=int,default=42)
    ...
    p.add_argument("--num-molecules",type=int,default=10); p.add_argument("--num-atoms",type=int,default=20)
    ...
    p.add_argument("--num-molecules",type=int,default=10); p.add_argument("--max-atoms",type=int,default=38)
    p.add_argument("--temperature",type=float,default=1.0)
    p.set_defaults(func=cmd_generate_gflownet)

    p=subparsers.add_parser("dock",help="Molecular docking")
    p.add_argument("--receptor",required=True); p.add_argument("--ligands",nargs="+",required=True)
    ...
    p.add_argument("smiles",nargs="+"); p.add_argument("--endpoints",nargs="+",default=["solubility","herg_inhibition","bioavailability"])
...
def cmd_train_advanced(args):
    ...
    config = AdvancedTrainingConfig(epochs=args.epochs, learning_rate=args.lr, use_amp=args.use_amp, use_ema=args.use_ema, scheduler=args.scheduler)
...
def cmd_evaluate_scientific(args):
    from drug_discovery.validation.scientific_validation import set_global_seed, ExperimentReport
